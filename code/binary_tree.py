...
from code.logical_layer import ValueRef, LogicalBase
...
class BinaryNodeRef(ValueRef):
    ...
    def length(self):
        if self._referent is not None and self._address:
            raise RuntimeError("Asking for BinaryNodeRef length of uploaded node")
        if self._referent:
            return self._referent.lenght
        else:
            return 0
...
class BinaryTree(LogicalBase):
    ...
    def _delete(self, node, key):
        if node is None:
            ...
            new_node = BinaryNode.from_node(node, left_ref=self._delete(self._follow(node.right_ref), key))
        elif node.key < key:
            ...
            if left and right:
                ...
                new_node = BinaryNode.from_node(left_ref,
                                                replacement.key,
